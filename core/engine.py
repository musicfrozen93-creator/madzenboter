...
from signals.btc_regime import BTCRegimeFilter
...
class TradingEngine:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # Global BTC-regime participation filter (read-only public market data).
        # A directional gate applied to signals before fan-out: blocks only
        # counter-trend entries during a strong BTC trend; permissive otherwise.
        self.btc_regime = BTCRegimeFilter(self.exchange_client, settings)
    ...
    def _run_loop(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                # Refresh the global BTC regime once per loop (cached internally,
                # only re-classifies every btc_regime_refresh_seconds).
                btc_regime = self.btc_regime.refresh()
                ...
                for coin in self._watchlist:
                    try:
                        ...
                        # Global BTC-regime gate: permissive in SIDEWAYS, blocks
                        # only counter-trend entries during a strong BTC trend.
                        btc_ok, btc_reason = self.btc_regime.allows(sig.side)
                        if not btc_ok:
                            blocked_btc += 1
                            logger.info(
                                'SIGNAL_REJECTED %s | stage=btc_regime | %s',
                                coin.symbol, btc_reason,
                            )
                            continue
                ...
                if evaluated:
                    logger.info(
                        ...
                        'rejected_at_signal=%d btc_regime=%s blocked_by_btc=%d '
                        '(per-account accept/reject logged above)',
                        len(self._watchlist), evaluated, found, evaluated - found,
                        btc_regime.value, blocked_btc,
                    )
