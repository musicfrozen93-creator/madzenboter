...
class AccountDatabaseWrapper:
    ...
    def get_today_trades(self, account_id: Optional[int] = None) -> List[TradeRecord]:
        # Force the current account's trades (used by RiskManager daily PnL).
        return self._db.get_today_trades(account_id=self._account_id)
...
class SignalExecutor:
    ...
    def _construct_account_components(
        ...
        try:
            ...
            # The only honoured per-account override is the admin leverage
            # override (clamped to 3x–8x, hard cap 10x). Sizing, limits, and
            # targets are globally fixed — every account uses the same config.
            overrides = {'leverage_override': account.leverage_override}
            acct_settings = Settings.create_account_settings(self.master_settings, overrides)
            acct_settings.use_testnet = account.use_testnet
