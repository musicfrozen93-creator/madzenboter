...
class Settings:
    ...
    # After a basket on a symbol closes (for ANY exit reason), that same symbol
    # cannot be re-opened for this many minutes. Prevents the rapid
    # close→reopen→loss churn observed on volatile symbols (e.g. AIOUSDT).
    # The cooldown is account-specific (keyed per account in bot_state) and
    # survives bot restart. Set to 0 to disable. Other symbols are unaffected.
    symbol_cooldown_minutes: float = 30.0

    # ── BTC Regime Participation Filter (global directional gate) ──
    # Layered on top of per-symbol entry logic. Blocks ONLY counter-trend
    # entries during a confirmed strong BTC trend; fully permissive (both
    # directions allowed) when BTC is ranging/SIDEWAYS, to maximise
    # participation in ranging markets. Set enabled=False to disable entirely.
    btc_regime_enabled: bool = True
    btc_regime_symbol: str = 'BTC/USDT:USDT'
    btc_regime_refresh_seconds: int = 300  # BTC 1h trend changes slowly
