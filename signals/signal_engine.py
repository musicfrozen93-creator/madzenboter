"""
Zentry — Mean-Reversion Setup Detector.

Detects mean-reversion setups on the analysed timeframe:

  LONG  : RSI(14) < 30  AND price touches the LOWER Bollinger band  AND benchmark allows
  SHORT : RSI(14) > 70  AND price touches the UPPER Bollinger band  AND benchmark allows

Market-quality filters (spread, ATR explosion, news candle, volume spike) can
withhold a setup; they are shared with the analysis pipeline via
`signals.filters`. The benchmark trend filter gates the allowed direction.

Consumes a `MarketDataProvider`, not an exchange client, so it works against any
market the provider layer supports.
"""
...
from signals.filters import evaluate_market_quality, first_failure
...
class SignalEngine:
    """Detects mean-reversion setups for a symbol on a timeframe.
    ...
    plus the benchmark trend filter must permit the direction.

    Market-quality filters (any one trips → skip with a logged reason):
    ...
    def __init__(self, provider: MarketDataProvider, settings: Settings) -> None:
        ...
        # Cached benchmark regime (recomputed at most every btc_regime_cache_seconds).
    ...
    def get_btc_regime(self) -> BtcRegime:
        """Return the market benchmark's trend regime, cached for the configured TTL.

        On any failure it fails safe by returning UNKNOWN (which allows both
        directions).
        """
        now = time.time()
        ...
        benchmark = self.provider.reference_symbol()
        try:
            if not benchmark:
                raise ValueError('provider exposes no benchmark symbol')
            df_btc = self.provider.fetch_candles(
                benchmark, self.settings.timeframe,
                limit=self.settings.candle_limit,
            )
            regime = classify_btc_regime(df_btc, self.settings)
        except Exception as e:
            logger.warning(
                'Benchmark regime fetch failed for %s: %s — failing safe (UNKNOWN, '
                'both directions allowed)', benchmark, e,
            )
            regime = BtcRegime.UNKNOWN

        self._btc_regime = regime
        self._btc_regime_ts = now
        logger.info('BENCHMARK_FILTER | %s (cached %ds)', regime.value.upper(), ttl)
        return regime

    # ───────────────────────────────────────────
    # Market-quality skip filters
    # ───────────────────────────────────────────

    def _risk_filter_reason(
        self, df: pd.DataFrame, atr: float, spread: float, price: float
    ) -> Optional[str]:
        """Return a skip reason if any market-quality filter trips, else None.

        Delegates to `signals.filters` so the pipeline and this detector apply
        identical thresholds.
        """
        failure = first_failure(
            evaluate_market_quality(df, atr, spread, price, self.settings)
        )
    ...
    def generate_signal(
        ...
        """Detect a mean-reversion setup for a symbol on a timeframe.

        Which symbols exist is the provider's authority, not this engine's — a
        symbol the venue does not list raises from `fetch_candles` and is
        reported as a skip.

        Args:
            symbol: Platform symbol (e.g. 'TRXUSDT').
            timeframe: Candle interval; defaults to the configured timeframe.

        Returns:
            Signal if all entry conditions hold and no quality filter trips,
            None otherwise (the reason is logged as SIGNAL_SKIP).
        """
        s = self.settings
        interval = timeframe or s.timeframe

        try:
            df = self.provider.fetch_candles(symbol, interval, limit=s.candle_limit)
            ...
            if s.btc_filter_enabled:
                ...
                if not regime_allows_side(btc_regime, side):
                    logger.info(
                        'SIGNAL_SKIP | symbol=%s direction=%s reason=benchmark_filter '
                        '(benchmark %s blocks %s) rsi=%.1f price=%.6f',
            ...
            logger.info(
                ...
                'benchmark=%s score=%d/4 [%s] strength=%.2f | reason: %s',
