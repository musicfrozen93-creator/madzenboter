...
balance — so deposits/withdrawals can never move it. Tier 1 limits are +$2/−$3,
Tier 2 limits are +$3.5/−$4.
"""
...
def test_tier1_profit_target_blocks_new_entries(settings: Settings, fake_db):
    tier1 = settings.get_tier(25.0)            # target $2
    rm = _rm(settings, fake_db, balance=25.0)
    fake_db.today_trades = [_trade(1.5), _trade(0.6)]  # realised $2.10 ≥ $2
...
def test_profit_target_includes_unrealized(settings: Settings, fake_db):
    tier1 = settings.get_tier(25.0)            # target $2
    rm = _rm(settings, fake_db, balance=25.0)
    # No realised PnL, but open baskets are up $2.2 → still latches.
    assert rm.update_profit_target(2.2, tier1) is True


def test_tier1_profit_target_not_reached(settings: Settings, fake_db):
    tier1 = settings.get_tier(25.0)
    rm = _rm(settings, fake_db, balance=25.0)
    fake_db.today_trades = [_trade(0.5)]
    assert rm.update_profit_target(1.0, tier1) is False   # total $1.50 < $2
    assert rm.can_take_new_entry()[0]
...
def test_tier2_uses_four_dollar_limits(settings: Settings, fake_db):
    tier2 = settings.get_tier(100.0)           # profit $3.5 / loss $4
    rm = _rm(settings, fake_db, balance=100.0)
    assert rm.update_profit_target(3.0, tier2) is False   # < $3.5 profit target
    assert rm.update_profit_target(3.5, tier2) is True    # hits the $3.5 profit target
